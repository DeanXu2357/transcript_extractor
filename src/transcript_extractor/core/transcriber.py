...
# Disable TF32 for reproducibility and compatibility (as per pyannote.audio recommendation)
        ...
        if self.model is None:
            try:
                ...
                if "libctranslate2" in str(e) and self.device != "cpu":
                    print(f"Warning: GPU model loading failed ({e}), falling back to CPU with float32")
                    self.device = "cpu"
