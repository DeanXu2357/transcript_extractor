    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for name, url in TEST_URLS.items():
            ...
            try:
                ...
                if transcripts:
                    print(
                        f"    ✓ Found transcripts in languages: {list(transcripts.keys())}"
                    )

                    for lang, content in transcripts.items():
                        if content and len(content.strip()) > 0:
                            print(
                                f"    ✓ {lang} transcript has content ({len(content)} chars)"
                            )
        ...
        try:
            ...
            if result.success:
                ...
                if result.youtube_transcripts:
                    print(
                        f"    ✓ YouTube transcripts: {list(result.youtube_transcripts.keys())}"
                    )
                else:
                ...
                if result.raw_result and isinstance(result.raw_result, dict):
                    print(
                        f"    ✓ Raw result available with {len(result.raw_result)} keys"
                    )
                ...
                if (
                    ...
                    and result.transcript_srt == ""
                    and result.transcript_vtt == ""
                ):
